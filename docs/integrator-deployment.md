...
**Audience**: operators deploying and running the reference integrator (`@urtela/pm-integrator`) for a PM project.
...
4. **Build:** `pnpm install && pnpm --filter @urtela/pm-integrator build`.
5. **Run:**
   ```bash
...
- **Auto-rollforward** (the common path): the incident is **reconcilable** when the current outer gitlink is an _ancestor_ of `O` (checked in the inner repo). The integrator assembles the roll-forward outer tree (gitlink → `O`, inner sources materialized), **verifies it** (the safety gate), then does a verify-gated fast-forward push of outer `main`, and resolves the incident `auto_resolved`. Outer `main` advances **only** by a verified, fast-forward push.
  ...
  > **Git LFS (closed):** the roll-forward assemble is now **LFS-aware**, exactly as the LAND assemble. Recovery **checks `O` out in the inner worktree** (so the inner clone's LFS filter smudges `O`'s correct binaries) and then materializes the inner sources **with** the LFS overlay — the real inner LFS binaries are copied into the outer working tree before verify. A **fail-loud guard** refuses to overlay an **unsmudged pointer** (it throws rather than silently rolling a pointer forward), and the whole assemble window is wrapped so **any** materialize failure (smudge 404, the fail-loud guard, a bad object) **escalates cleanly** — the incident stays `open`, nothing is pushed, and a human completes the roll-forward. An orphan whose inner sources **can** be materialized as real binaries auto-rolls-forward with those binaries; one that cannot escalates (a pointer is never rolled forward).
...
| Inner-only group: real member binds OUTER                          | A `synthesize_outer` group rejects from `forming` with "outer-only changes don't need a group; submit a plain merge request" | Clean reject, nothing leased or pushed.                                                                                                                                                                                                                                                      | Worker submits the outer change as a plain `pm_request_merge` (or fixes the ref to point at the inner change). |
...
`160000` gitlink path is skipped by `git status`, `git clean -fdx`, and `git reset --hard`.
The linked outer pool retains its prior overlay and materialize replaces it through a staged
**tree-exact, content-stable sync**: destinations absent from the new tree are deleted, changed
files are copied, and byte-identical files are not rewritten. This is stale-file safe and preserves
the mtimes needed by timestamp-based incremental compilers. Pools that do not run the materializer
continue to purge declared gitlink overlays on reset; that purge is triple-guarded and only removes
a populated, `.git`-less directory at a real `160000` gitlink. A real initialized submodule checkout
or tracked directory is never touched. Hand-cleaning a pool slot is not needed.
...
`branch`/`commit_sha`/`task_id`/`verify_cmd` are all null, flagged `synthetic: true`. The flag is
additive on every view — real members carry `synthetic: false` — so existing tooling is unchanged.

- **Dashboard / timeline.** The synthetic member renders a "synthetic gitlink bump" badge in place
  of a branch/SHA; `pm_get_merge_group` renders it as "(synthetic gitlink bump — outer candidate
  synthesized at integration)". At land the integrator fills its `landedSha` with the synthesized
  outer commit `Ro` — auditable like any member.
- **Assembly.** The synthetic member is the outer **by construction** — PM mints it ref-less and it
  is never ref-resolved; the real member's ref must resolve in the inner repo (lane roles are
  config-declared via `linked_repos`). The outer rebase is **skipped** — the outer candidate is
  live outer `main` + a gitlink commit → `Ri`. Everything from the gitlink commit on (materialize,
  the §14.8 overlay, verify, inner-first land, orphan recovery) is the unchanged §14.3 path.
  Consequence: **`outer_conflict` cannot occur for an inner-only group** — there is nothing to
  rebase.
- **No-op behavior.** When the inner change is already on main and the gitlink already points at
  `Ri`, the gitlink commit is **skipped at assembly** (the gitlink-update step's no-change
  idempotence is explicit and contractual — the train never mints an empty bump commit) and the
  land's fast-forward push is a natural up-to-date no-op. There is no separate short-circuit code
  path.
- **Verify.** The synthetic member has no `verify_cmd`, so it runs the project default
  `verify_command` in the **outer** worktree against the assembled state — identical to what a real
  bump member got. The §14.8 verify-command contract applies unchanged.
- **Topology prerequisite.** `synthesize_outer` is a 400 unless `linked_repos` declares exactly one
  inner + one outer. Only the DECLARED gitlink is synthesized — game_one's second gitlink
  `tools/rynx-treegen` still needs a real outer member via the multi-member form.
- **Forming-reject reasons an operator may see.** All reject the group from `forming` (no worktrees
  leased); the worker fixes and resubmits the one member: the real member binds OUTER (reason ends
  "outer-only changes don't need a group; submit a plain merge request" — a worker error with
...
| **Force-land**         | `POST /api/v1/merge-requests/{id}/force-land`                            | `{ landedSha, reason }`  | **REQUIRED** (400 if empty) | **THE R1 override** — see §15.3.1. Group-state-aware: a `rejected` member of a **terminal** group (`rejected`/`partially_landed`) is also force-landable (§15.3.2); a member of a live or landed group → 409.                                                                                                                                                                |
...
- **Precondition (legality matrix, group-state-aware):**
  - **Non-grouped:** the request must be `integrating` (force-landing `landed` is an idempotent 200 no-op; `queued`/`rejected`/`abandoned` → 409). Unchanged from 7.4.
  - **Grouped, member already `landed`:** idempotent 200 no-op, no audit (evaluated before any group check).
  - **Grouped, group `forming`/`integrating`/`landed`:** → **409 GROUPED_MEMBER** (a live group's member can only land via the group's atomic land; a landed group has nothing to fix).
  - **Grouped, group terminal (`rejected`/`partially_landed`):** the member is force-landable **only from `rejected`** (the stuck-member shape). An `orphaned`-marked inner member, or a `queued`/`abandoned` member → 409 — `orphaned` means inner main already holds the SHA; recovery (rollforward) owns it, not force-land.
  - Force-landing the **last** stuck member of a `partially_landed` group auto-resolves the group's open incidents as `human_resolved` in the same transaction (see §15.3.2).
...
#### 15.3.2 Recovering a partially-landed group

A `partially_landed` group is the cross-repo failure shape from §9: the inner repo's commit landed (and may be marked `orphaned`) but the outer repo's land failed, leaving one or more members `rejected` and an open `orphaned_inner` incident. Auto-rollforward (§9) is the preferred recovery — it is verify-gated and needs no human. Use this runbook only when rollforward cannot reconcile (the incident escalated, or you must move NOW).

1. **Stop the integrator racing you:** `pause` the lane (and/or `force-release-lock` if a dead integrator still holds it). PM never runs git — every git step below is yours.
2. **Reconcile git manually:** on the outer repo, advance the gitlink to the orphaned inner SHA (or otherwise reconcile both repos), run your verify out-of-band if you want any safety net, and fast-forward the outer remote `main` to the resulting SHA. This is exactly the manual half of the force-land contract (§15.3.1): PM will only *record* what you assert.
3. **Force-land the stuck member(s):** `POST /api/v1/merge-requests/{id}/force-land` with `landedSha` = the outer SHA you just pushed and a reason. Each stuck member must be `rejected` and its group terminal (`rejected`/`partially_landed`) — anything else 409s (see the matrix above).
4. **Incident auto-resolves:** when the member you force-land is the **last** `rejected` member of a `partially_landed` group, PM resolves every open incident bound to that group as `human_resolved` in the **same transaction** (resolution note records the force-land id + reason; `outerLandedSha` = your asserted SHA), posts the follow-up `merge_incident` comment on the linked task, and emits `merge.incident.human_resolved`. No separate incident-resolve call is needed.
5. **Verify on the dashboard:** the train dashboard should show the member `landed` (badged as overridden), the incident `human_resolved`, and one `force_land` audit row whose `after` payload carries `groupId`/`groupState`/`resolvedIncidentIds`. Then `resume` the lane.

If other `rejected` members of the same group remain, the incident **stays open** — repeat step 3 per member; the resolve fires only with the last one.
  ...
  - Or console:
    ...
    for (const id of ["{id1}", "{id2}"]) {
      const r = await fetch(`/api/v1/merge-requests/${id}/force-cancel`, {
        ...
        credentials: "include",
        body: JSON.stringify({
          reason: "content hand-landed out-of-band; clearing stale queued entry",
        }),
      });
...
3. Build + run on the integrator host:
   ```bash
   pnpm install && pnpm --filter @urtela/pm-integrator build
...
This is **opt-in**. With `settings.integrator.resolver.enabled = false` (the default) the train is **byte-identical to 7.5**: a conflict rejects exactly as before, zero resolution rows are written, zero resolution events fire. This section is the operator-facing summary; the authoritative spec is `docs/design/phase-7.6-design.md`, and the **in-session resolver loop** follow-up (Phase 7.6.1 — the agent owns verification, a status sentinel, and a periodic reclaim sweep; see §18.7) is specified in `docs/design/phase-7.6.1-resolver-in-session-loop.md`.
...
| `time_budget_sec` | number > 0  | `3600`  | Wall-clock cap on the **whole resolution session** (SIGTERM then SIGKILL). In 7.6.1 the agent runs the full verify loop in-session, so this bounds resolve→verify→fix end-to-end, not a single agent turn. Exceeding ⇒ escalate. See §18.3.    |
...
These are the resolver fields. An **absent/empty** `resolver` block is treated as `{ enabled: false, max_concurrent: 1, time_budget_sec: 3600 }` (the inner defaults fire). Configure it in one of two ways:
...
A resolution is a single **in-session loop** (Phase 7.6.1): the agent resolves, runs the full verify itself, and iterates fix→verify to a green suite **within one session**, bounded by the whole-session budget — there is **no daemon-side retry loop**. Three controls bound its cost:

- **`time_budget_sec`** (default `3600`) — the wall-clock cap on the **whole resolution session** (the in-session resolve→verify→fix loop, not a single agent turn). When it's exceeded the session is killed (SIGTERM, then SIGKILL after a grace period) and the resolution **escalates** to the author. A resolution is never allowed to run unbounded. **Size it generously** — at least a few times your verify duration, since the agent runs the full verify in-session (possibly more than once).
...
1. **Auto-resolve.** The resolver reconciles the conflict in an isolated worktree and — in the 7.6.1 in-session loop — **the agent runs the full verify itself**, iterating fix→verify until the suite is green, then declares completion via the status sentinel. The daemon no longer runs its own verify gate; it commits the agent's declared-green tree and resubmits it as a **new** merge request (`resolved_from = origin.id`, `task_id` copied from the origin). That resolved request **rides the train and passes the REAL verify gate again** before it can land — the **train re-verify is the SOLE landing gate** (the agent's in-session verify is the agent's own loop discipline, not the authority). On a clean pass it lands; `merge_resolutions.state = resolved`.
...
- **Dashboard metrics sub-block.** The on-read metrics bundle (`GET .../train/metrics`) gains a `resolution` sub-block (snake_case, inert — `attempts: 0`, ratios `null` — when the resolver is off):
  ...
  ```json
  {
    "resolution": {
      ...
      "mean_session_sec": 184,
      "reclaimed_count": 1,
      "budget_utilization": { "ratio": 0.05, "mean_consumed_sec": 184, "budget_sec": 3600 }
    }
  }
  ```
...
- **Stranded `resolving` rows are reclaimed (7.6.1).** If the integrator **crashes mid-resolution**, the `merge_resolutions` row is stranded in `resolving`. **No work is lost** (the origin commit is intact and `main` is untouched), and the periodic **reclaim sweep** (§18.7) now recovers it: past the deadline (`attempt_started_at + time_budget_sec + grace`) it reconciles-or-escalates the row, closing the v1 dangling-`resolving` gap. An operator can still spot live `resolving` rows via `GET .../merge-resolutions`.
- **The honest limitation.** Verify is the **only** arbiter — the resolver's self-asserted confidence never gates a land (the train re-verify is the sole landing gate). In 7.6.1 the agent **iterates resolve→verify→fix in-session** (no longer strictly one-shot), still bounded by the whole-session budget and still a single resubmission; resolution remains **conflict-only** (semantic verify failures and cross-repo group conflicts are out of scope; group conflicts stay on the 7.3 reject/incident path). The prime invariant holds: **`main` is only ever advanced by the normal verify-gated land path** — the resolver only ever _submits a request_, never pushes `main` itself.

### 18.7 In-session resolver loop (Phase 7.6.1)

7.6.1 moves verification **into the resolver session** and adds a recovery sweep, without changing the train's land gate. Spec: `docs/design/phase-7.6.1-resolver-in-session-loop.md`.

- **The agent owns verification.** Within its single bounded session the headless agent reconciles the conflict, **runs the full verify itself**, and iterates `resolve → verify → fix` until the **whole suite is green**, then declares completion. The daemon **dropped its own verify gate** — it no longer runs a pre-filter verify on the resolved tree; it commits the agent's declared-green tree and resubmits.
- **Status sentinel.** The agent signals its terminal state by writing a JSON file at `PM_RESOLUTION_STATUS_PATH` (injected into the agent env; the path lives **outside** the git worktree so it never registers as a tree change): `{"status":"complete"}` ⇒ the agent verified green in-session; `{"status":"give_up","reason":...}` ⇒ it needs an author decision. An **absent / unparseable / residual-marker** sentinel ⇒ the daemon treats it as `incomplete` and **escalates** (`complete` is never inferred from a clean tree).
- **The train re-verify is the SOLE landing gate.** The resubmitted resolution request re-enters the train and **passes the real verify gate against live `main`** before it can land. The agent's in-session verify is its own loop discipline, never the land authority — so an agent that wrongly declares `complete` simply fails the train re-verify and rejects/escalates; `main` is never at risk.
...
### 20.6 Auto-implement land path (Campaign A1+A2)

Beyond answer/diagnose, the responder can **autonomously land a code fix** for a **bounded** escalation — gated by the responder config `auto_implement.enabled` (**default FALSE**; ships OFF, like the responder itself). When enabled, a clean **injection sniff-test** gates admission (suspicious/un-runnable → escalate-to-human, never spawn — fail-safe); a session that declares `implement{bounded}` runs in an **isolated worktree clone** (never `main`), and on a commit-cross-checked `branch_ready` the loop applies a **coarse path-prefix allowlist** (empty = whole PM repo), pushes `pm/escalation-<id>`, and **submits a task-less, `escalationId`-linked merge request over HTTP**. The **merge train lands it verify-gated** — a wrong diff is caught by verify and never lands (`main` structurally safe). On **land** the responder (as the escalation holder) posts the `landed_sha` summary back and resolves the escalation (the origin auto-notices via C2); on **reject** the escalation goes to `needs_human` with the reject reason and the branch is preserved. When the 7.6 conflict resolver reconciles a responder MR it propagates `escalationId` (alongside `resolvedFrom`) so the post-back still fires on the resolution's land. **Discipline:** bring up `auto_implement` only after the responder itself is trusted at `on`; it ships OFF and is additive (with it off, the responder is byte-identical to C3). Full description in `CLAUDE.md` (canonical).
