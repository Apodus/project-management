...
> **CONSOLIDATION (user-directed, 2026-06-03):** C1 + C2 are SHIPPED. After seeing the live DAG, the user directed that **C3 and C4 merge into one campaign — "C3′ — Dashboard-as-DAG-hero + board re-scope"** — with one deliberate change to original C3: **the dashboard STAYS the default landing** (we do NOT flip the default route to a separate `/roadmap` page). Instead the **dashboard itself becomes the DAG-hero surface**: the epic DAG is embedded as the hero at the top of the dashboard, the pulse widgets (attention/my-work/AI-agents) become the thin dressing rail around it (the C4 work, already partly shipped as C4.P1 density), and the board is re-scoped into an epic drill-down (the valuable part of original C3 that kills the "two infinite lists"). The standalone `/roadmap` route remains as a focused full-bleed view. This collapses the artificial C3-route-page / C4-dashboard split into a single coherent hero surface. **Driven via /campaign 2026-06-03 — see `roadmaps/roadmap-20260603-c3prime-dashboard-dag-hero.md`.** Original C3/C4 sections below are RETAINED for reference but superseded by C3′.
