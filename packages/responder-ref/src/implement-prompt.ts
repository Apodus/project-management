 ...
 *
 * `buildImplementPrompt` substitutes three `{placeholder}` blocks into a template
 * (default `DEFAULT_IMPLEMENT_PROMPT`): `{escalation}`, `{thread}`, and `{branch}`.
 * Substitution is replace-if-present (parity with `buildResponderPrompt`): a custom
 * template that omits a placeholder simply does not receive that block.
 *
 ...
 *
 * NO in-session verify instruction yet — that arrives in P3 (the agent will be told
 * to run the full verify itself and iterate to green before declaring branch_ready).
 */
...
/**
 * Build the implement prompt by substituting the `{escalation}`, `{thread}`, and
 * `{branch}` blocks into `template ?? DEFAULT_IMPLEMENT_PROMPT`. Replace-if-present:
 * a template omitting a placeholder simply does not receive that block.
 */
