...
describe("migration log heal + boot assertion", () => {
  ...
  it("REGRESSION (the live incident): future watermark + skipped migration → boot heals, applies, and ends current", () => {
    ...
    // effect. The CURRENT last migration (0038) IS a schema migration
    // (CREATE TABLE merge_phase_timings), so this simulation unrecords just the
    // last entry, reverts it (DROP TABLE), and proves boot heals the watermark
    // then re-applies it. When the last migration is instead a DATA migration
    // with no schema effect to revert, unrecord back through the last SCHEMA
    // migration (update LAST_REVERT + LAST_COLUMN_CHECK and the delete count to
    // cover it) — this test fails loudly (duplicate table/column on re-apply,
    // or a missing column) if they fall behind.
    const LAST_REVERT = "DROP TABLE merge_phase_timings;"; // 0038
    const LAST_COLUMN_CHECK = {
      table: "merge_phase_timings",
      column: "phase",
    };
    const raw = new Database(dbPath);
    // The genuinely-last recorded migration(s) — back through the last schema
    // migration (here just 0038, itself a schema migration).
    ...
    const last = unrecorded[0]!; // 0038 — the genuinely-last entry
