...
describe("POST /api/v1/merge-requests/:id/outer-converted", () => {
  let testApp: TestApp;
  ...
  it("200: ai_agent writes exactly one `outer_converted` audit row, NO status change", async () => {
    ...
    const res = await authRequest(
      testApp.app,
      "POST",
      `/api/v1/merge-requests/${requestId}/outer-converted`,
      { token: agent.token, body: { reason: "pure gitlink bump — synthesized on live main" } },
    );
    ...
    const converted = audits.filter((a) => a.action === "outer_converted");
    expect(converted.length).toBe(1);
    expect(converted[0].targetType).toBe("merge_request");
    expect(converted[0].actorId).toBe(agent.user.id);
    expect(converted[0].reason).toBe("pure gitlink bump — synthesized on live main");
    expect(converted[0].metadataBefore).toBeNull();
    expect(converted[0].metadataAfter).toEqual({ converted: true });
  });
  ...
  it("403: human (non-ai_agent) is forbidden", async () => {
    ...
    const audits = testApp.db.select().from(auditLog).where(eq(auditLog.targetId, requestId)).all();
    expect(audits.filter((a) => a.action === "outer_converted").length).toBe(0);
  });
  ...
  it("404: missing request", async () => {
    ...
    const res = await authRequest(
      testApp.app,
      "POST",
      `/api/v1/merge-requests/${createId()}/outer-converted`,
  ...
  it("401: unauthenticated", async () => {
    ...
    const res = await testApp.app.request(`/api/v1/merge-requests/${requestId}/outer-converted`, {
      method: "POST",
