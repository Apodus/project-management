...
test.describe("Search via Command Palette", () => {
  ...
  test("Ctrl+K opens command palette, search finds task, click navigates", async ({
    page,
  }) => {
    await login(page, ADMIN_USER, ADMIN_PASS);
    ...
    await expect(
      page.getByPlaceholder(
        "Search tasks, proposals, or type a command...",
      ),
    ).toBeVisible({ timeout: 5_000 });

    // Type a search query
    await page
      .getByPlaceholder("Search tasks, proposals, or type a command...")
      .fill("dark mode");

    // Wait for debounced search results to appear in the command palette
    const commandDialog = page.locator('[role="dialog"]');
    await expect(
      commandDialog.getByText("Implement dark mode toggle"),
    ...
    await expect(
      page.getByText("Implement dark mode toggle"),
    ).toBeVisible();
  });
